...
- 后端完整回归：基于最新 `origin/main` 的全新临时 SQLite 数据库，`463 tests collected`，进度到 `[100%]`，退出码 `0`。
- 前端单元测试：基于最新 `origin/main`，`20 passed` 测试文件，`179 passed` 测试。
...
- 并行改动处理：合并时以已进入 `origin/main` 的“提取渠道指纹”入口移除及其回归测试为准，不恢复旧入口；用户已有的 2026-08-12 checklist 修改未纳入本任务提交。
