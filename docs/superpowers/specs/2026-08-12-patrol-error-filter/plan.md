...
本次改动限定在前端自动巡检日志集合筛选、工具栏交互和紧凑单元格展示，不修改后端接口、数据库或现有巡检证据分类。页面先按现有渠道条件得到巡检日志集合，再按“只看错误”开关调用同一套 `patrolEvidenceDisplayState` 判断结果状态，最后继续使用现有分页、行选择和删除逻辑。紧凑单元格只渲染状态与探针标签，不渲染错误说明正文；详情证据组件保持不变。
...
**职责：** 管理“只看错误”开关，准备当前日志的证据状态缓存，按渠道和错误条件生成列表，并把同一筛选集合用于分页、选择和删除；紧凑结果单元格只保留状态与探针标签，隐藏兼容说明、relay 错误、500 摘要等正文。
...
| 说明正文 | 默认列表隐藏，详情保留 | 保持表格紧凑，同时不丢失证据 |
