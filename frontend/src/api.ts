...
export const api = {
  ...
  channels: (groupId?: unknown) => request<Channel[]>(`/api/channels${queryString({ group_id: typeof groupId === 'string' ? groupId : undefined })}`),
  channelGroups: () => request<ChannelGroup[]>('/api/channel-groups'),
  createChannelGroup: (payload: ChannelGroupCreate) => request<ChannelGroup>('/api/channel-groups', { method: 'POST', body: JSON.stringify(payload) }),
  updateChannelGroup: (id: string, payload: ChannelGroupUpdate) => request<ChannelGroup>(`/api/channel-groups/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
  deleteChannelGroup: (id: string) => request<{ deleted: boolean; unlinked_channels: number }>(`/api/channel-groups/${id}`, { method: 'DELETE' }),
  replaceChannelGroups: (id: string, groupIds: string[]) => request<Channel>(`/api/channels/${id}/groups`, { method: 'PUT', body: JSON.stringify({ group_ids: groupIds }) }),
  ...
  claudeCodeTest: (channelId: string, payload: { source_channel_id?: string | null; image_url?: string | null; include_expensive_context?: boolean; probe_depth?: 'standard' | 'deep'; repeat_count?: 3 | 5; fast_mode_probe?: ClaudeCodeRelayTestCreate['fast_mode_probe'] }) =>
  ...
  deletePatrolScope: (payload: PatrolScopeDeleteRequest) => request<PatrolScopeDeleteResult>('/api/runs/patrol/delete-scope', { method: 'POST', headers: adminHeaders(), body: JSON.stringify(payload) }),
